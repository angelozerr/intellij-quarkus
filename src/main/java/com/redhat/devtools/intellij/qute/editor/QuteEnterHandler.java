 ...
 */
public class QuteEnterHandler extends EnterHandlerDelegateAdapter {
  ...
  public Result preprocessEnter(@NotNull final PsiFile file,
     ...
     */
    if (file instanceof QutePsiFile
  ...
  private static boolean isBetweenHbTags(Editor editor, PsiFile file, int offset) {
    ...
    PsiElement openTag = findParentOpenTagElement(openerElement);
    ...
    PsiElement closeTag = findParentCloseTagElement(closerElement);

    // if we got this far, we're between open and close tags iff this is a close tag
    return closeTag != null;
  }

  private static PsiElement findParentOpenTagElement(PsiElement openerElement) {
    if (openerElement instanceof QuteToken && (((QuteToken)openerElement).getElementType() == QuteTokenType.QUTE_START_TAG_CLOSE)) {
      return openerElement;
    }
    return null;
  }

  private static PsiElement findParentCloseTagElement(PsiElement closerElement) {
    if (closerElement instanceof QuteToken && (((QuteToken)closerElement).getElementType() == QuteTokenType.QUTE_START_TAG_CLOSE)) {
