...
public final class CompletionProposalTools {
    ...
    public static @Nullable String getCompletionPrefix(@NotNull Position completionPos, @NotNull Range textEditRange, @NotNull Document document) {
        var textEditStart = textEditRange.getStart();
        var textEditEnd = textEditRange.getEnd();
        if (textEditStart.getLine() == textEditEnd.getLine() // the text edit range covers the same line
                && textEditStart.getLine() == completionPos.getLine() // the text edit range is on the same line that completion position
                && textEditStart.getCharacter() <= completionPos.getCharacter() // and the completion is triggered inside the text edit
                && textEditEnd.getCharacter() >= completionPos.getCharacter()) {
            // ex : {#ea|ch
